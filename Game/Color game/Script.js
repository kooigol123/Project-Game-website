class Game {
  ...
  intro() {
    ...
    let ball = new TimelineMax({ repeat: -1, delay: 30 });
    introTl.
    fromTo('.start-game .logo-holder', 0.2, { opacity: 0 }, { opacity: 1 }).
  ...
  * Display score
  ...
  moveToStart() {
    ...
    let tip = new TimelineMax({ delay: 1 });

    tip.
    fromTo('.learn-to-play', 1, { scale: 0 }, { scale: 1, opacity: 1, ease: Elastic.easeOut.config(1.25, 0.5) }).
    to('.learn-to-play', 10, { scale: 0, opacity: 0, ease: Elastic.easeOut.config(1.25, 0.5) }, 3);
...
function Menu() {
  ...
  window.onclick = function(event) {
                          if (!event.target.matches('.bar2')) {
                          var dropdowns = document.getElementsByClassName("dropdown-bar2");
                          var i;
